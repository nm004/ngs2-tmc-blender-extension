...
class ObjGeoParser(ContainerParser):
    ...
    def _gen_texture_maps(texture_map_info):
        ...
        for m in texture_map_info:
            ...
            version = int.from_bytes(m[0x10:0x14], 'little')
...
class NodeObjParser(ContainerParser):
    ...
    def __init__(self, data):
        ...
        node_idx = int.from_bytes(self._metadata[0x8:0xc], 'little')
        name = bytes(self._metadata[0x10:]).partition(b'\x00')[0]
        self.metadata = NodeObjMetaData(unknown0x0, master, node_idx, name)
        if self._chunks:
            c = self._chunks[0]
            obj_idx = int.from_bytes(c[0x0:0x4], 'little', signed=True)
            group_size = int.from_bytes(c[0x4:0x8], 'little', signed=True)
            node_idx = int.from_bytes(c[0x8:0xc], 'little', signed=True)
            matrix = tuple( c[0x10:0x50].cast('f')[i:i+4] for i in range(0,16,4) )
            node_group = tuple(c[0x50:0x50+4*group_size].cast('i'))
            self.chunks = (NodeObjChunk(obj_idx, node_idx, matrix, node_group),)
